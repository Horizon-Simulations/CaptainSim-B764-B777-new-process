...
import { MathUtils, UnitType, Vec3Math } from '../math';
    ...
    build(vectors, index, start, startPath, startTurnRadius, startTurnDirection, interceptAngle, end, endPath, endTurnRadius, startTurnVectorFlags = 0, interceptVectorFlags = 0, endTurnVectorFlags = 0) {
        ...
        if (startTurnDirection === undefined) {
            ...
            if (interceptCount === 0) {
                ...
                target = Vec3Math.cross(intersections[0], endPath.center, InterceptGreatCircleToPointBuilder.vec3Cache[2]);
            }
        ...
        const interceptPath = InterceptGreatCircleToPointBuilder.geoCircleCache[1].set(intersections[interceptPathCount === 1 || !doesStartTurnOvershoot ? 0 : 1], MathUtils.HALF_PI);
        ...
        const intercept = Vec3Math.multScalar(Vec3Math.normalize(Vec3Math.cross(interceptPath.center, endPath.center, InterceptGreatCircleToPointBuilder.vec3Cache[3]), InterceptGreatCircleToPointBuilder.vec3Cache[3]), doesStartTurnOvershoot === (startTurnDirection === 'right') ? 1 : -1, InterceptGreatCircleToPointBuilder.vec3Cache[3]);
...
export class JoinGreatCircleToPointBuilder {
    ...
    build(vectors, index, start, startPath, end, endPath, desiredTurnDirection, minTurnRadius, preferSingleTurn = false, allowDirectFallback = true, intersection, flags = 0, includeTurnToCourseFlag = true, includeDirectFlag = true, includeInterceptFlag = true) {
        ...
        if (needCalculateTwoTurnPath) {
            ...
            const numInterceptVectors = this.interceptGreatCircleToPointBuilder.build(vectors, vectorIndex, start, startPath, minTurnRadius, desiredTurnDirection, JoinGreatCircleToPointBuilder.INTERCEPT_ANGLE, end, endPath, minTurnRadius, turnFlags | interceptFlag, flags | interceptFlag, turnFlags | interceptFlag);
